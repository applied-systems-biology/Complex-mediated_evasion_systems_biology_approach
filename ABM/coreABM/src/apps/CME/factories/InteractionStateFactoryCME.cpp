...
#include "InteractionStateFactoryCME.h"
...
InteractionStateFactoryCME::InteractionStateFactoryCME(const std::vector<std::unique_ptr<abm::util::SimulationParameters::InteractionParameters>> &interaction_parameters, RateFactory *rate_factory)
    : InteractionStateFactory(interaction_parameters, rate_factory){
    for (const auto &parameters: interaction_parameters) {
        for (const auto &state: parameters->states) {
            ...
            } else if (state.interaction_type == "ComplexFormation"){
                state_parameters_[parameters->name].emplace(state.name,
                                                            std::make_pair(ComplexFormation(), std::move(state_setup)));
            } else if (state.interaction_type == "ComplexExtension"){
                state_parameters_[parameters->name].emplace(state.name,
                                                            std::make_pair(ComplexExtension(), std::move(state_setup)));
            }else if (state.interaction_type == "Uptake"){
                state_parameters_[parameters->name].emplace(state.name,
                                                            std::make_pair(Uptake(), std::move(state_setup)));
            }
        }
    }
}

std::unique_ptr<InteractionState> InteractionStateFactoryCME::createInteractionState(Interaction *interaction,
    ...
    const auto clone = abm::util::overloaded{
            ...
            [&](RigidContacting type) -> std::unique_ptr<InteractionType> { return std::make_unique<RigidContacting>(type); },
            [&](Ingestion type) -> std::unique_ptr<InteractionType> { return std::make_unique<Ingestion>(type); },
            [&](ComplexFormation type) -> std::unique_ptr<InteractionType> { return std::make_unique<ComplexFormation>(type); },
            [&](ComplexExtension type) -> std::unique_ptr<InteractionType> { return std::make_unique<ComplexExtension>(type); },
            [&](Uptake type) -> std::unique_ptr<InteractionType> { return std::make_unique<Uptake>(type); },
    };
    ...
    if (intState->getInteractionType() == "Ingestion") {
        intState = std::make_unique<InteractionState>(interactionStateType, interaction, std::visit(clone, type),
                                                      false);
        if (interaction->getFirstCell()->getTypeName() == "ImmuneCell") {
            interaction->getFirstCell()->addIngestions(interaction->getSecondCell()->getId());
        } else if (interaction->getSecondCell()->getTypeName() == "ImmuneCell") {
